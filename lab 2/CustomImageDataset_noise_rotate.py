...
class CustomImageDataset_noise_rotate(Dataset):
    def __init__(self, annotations_file, img_dir, transform=None, target_transform=None):
        ...
        self.noise_transform = transforms.Lambda(lambda x: torch.clamp(x + torch.randn_like(x) * 0.4 * x.std(), x.min(), x.max()))
...
# image_set = CustomImageDataset_noise_rotate("train_noses.txt", "./images", transforms.Compose([transforms.Resize((227, 227))]), None)
# print(f"Dataset size: {len(image_set)}")
